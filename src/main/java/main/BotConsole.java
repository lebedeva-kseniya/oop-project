        ...
        while (true) {
            ...
            if (!isPlaying) {
                if (logicBot.canStartGame(answer)) {
                    console.dataOut("Как ты хочешь играть? 1 - классический режим, 0 - на время, 2 - на время со стандартными настройками");
                    while (game.getMode() == 3) {
                        console.dataOut(game.setModeGame(console.dataInput()));
                    }
                    console.dataOut("Какое слово ты хочешь отгадывать? Из 5, 6, или 7 букв? Ответь цифрой");
                    while (game.levelWord() == 0) {
                        console.dataOut(game.setDifficultWord(console.dataInput()));
                    }
                    console.dataOut("На какой сложности ты хочешь играть? 0 - простая, 1 - средняя, 2 - сложная, Ответь цифрой");
                    while (game.level() == 0) {
                    ...
                    console.dataOut(logicBot.handleUserAnswer(answer));
                }
            } else {
